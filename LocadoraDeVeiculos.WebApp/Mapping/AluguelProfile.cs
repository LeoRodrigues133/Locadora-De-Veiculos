...
public class AluguelProfile : Profile
{
    public AluguelProfile()
    {
        CreateMap<Aluguel, ListarAluguelViewModel>()
            .ForMember(vm => vm.Condutores, opt => opt.MapFrom(v => v.Condutor))
            .ForMember(vm => vm.Planos, opt => opt.MapFrom(v => v.Plano))
            .ForMember(vm => vm.Veiculos, opt => opt.MapFrom(v => v.Veiculo))
            .ForMember(vm => vm.Grupos, opt => opt.MapFrom(v => v.Grupo));
